// commands/fun/mem.js — v3: tablero siempre visible en el webview de WhatsApp
...
.mv-card { background: rgba(15,18,28,.95); border: 1px solid rgba(0,243,255,.25); border-radius: 16px; overflow: hidden; box-shadow: 0 8px 32px rgba(0,243,255,.15); }
.mv-header { padding: 12px 16px; border-bottom: 1px solid rgba(255,255,255,.1); display: flex; justify-content: space-between; align-items: center; }
.mv-title { font-size: 18px; font-weight: 900; color: #fff; letter-spacing: 1px; }
...
.mv-stat small { display: block; font-size: 9px; letter-spacing: 1.5px; color: #94a3b8; font-weight: 700; }
.mv-stat b { font-size: 14px; color: #fff; }
.mv-board { display: grid; gap: 6px; margin: 0 auto; width: fit-content; background: #1e293b; padding: 10px; border-radius: 10px; }
.mv-cell { width: 44px; height: 44px; background: #334155; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 800; color: #60a5fa; cursor: pointer; border: 2px solid #475569; }
.mv-cell.flip { background: #f8fafc; border-color: #94a3b8; color: #111; }
.mv-cell.gone { background: transparent; border-color: transparent; color: transparent; cursor: default; }
#mvStatus { margin: 12px 0; font-size: 15px; text-align: center; font-weight: 700; color: #8fc7ff; min-height: 20px; }
#mvNew { padding: 10px 20px; background: #22c55e; border: none; border-radius: 10px; color: #fff; font-size: 15px; font-weight: 800; cursor: pointer; display: none; margin: 0 auto; width: fit-content; }
</style>
<div class="mv-wrap">
  <div class="mv-card">
    <div class="mv-header">
...
function startGame(key){
  ...
  for (var k in btns){ if(k === key){ btns[k].classList.add('active'); } else { btns[k].classList.remove('active'); } }
...
function isGone(c){ return c.classList.contains('gone'); }
function available(){ return cards.filter(function(c){ return !isGone(c) && flipped.indexOf(c) === -1; }); }
function flipCard(card, player){
  if(over || locked || turn !== player) return;
  if(isGone(card) || card.classList.contains('flip')) return;
...
function checkMatch(player){
  ...
  if(c1.dataset.emoji === c2.dataset.emoji){
    ...
    c1.classList.add('gone'); c2.classList.add('gone');
