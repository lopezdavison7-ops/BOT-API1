    ...
    <div class="contenedor">
        ...
        <div class="tarjeta">
            ...
            <div class="ya-vinculado" id="yaVinculado">
                ...
                <div class="detalle">
                    Este dispositivo está autorizado para administrar los owners de este subbot.
                </div>
            ...
            <div class="panel" id="panel" style="display:none">
                <div class="aviso-sin-token" id="avisoSinToken" style="display:none">
                    🔒 Este navegador no tiene el token de este subbot.<br>
                    <b>Solo el dispositivo que lo vinculó originalmente puede ver y modificar sus owners.</b>
                </div>
...
const CLAVE_NUMERO = 'subbot_numero_recordado';

function claveToken(numero) { return 'subbot_token_' + numero; }

function guardarNumeroRecordado(numero) {
    try { localStorage.setItem(CLAVE_NUMERO, numero); } catch {}
}

function borrarNumeroRecordado() {
    try { localStorage.removeItem(CLAVE_NUMERO); } catch {}
}

function obtenerNumeroRecordado() {
    try { return localStorage.getItem(CLAVE_NUMERO); } catch { return null; }
}

function guardarTokenRecordado(numero, token) {
    if (!numero || !token) return;
    try { localStorage.setItem(claveToken(numero), token); } catch {}
}

function obtenerTokenRecordado(numero) {
    if (!numero) return null;
    try { return localStorage.getItem(claveToken(numero)); } catch { return null; }
}
...
function mostrarPantallaYaVinculado(id, estado, numero) {
    ...
    tokenActual = obtenerTokenRecordado(numero);

    if (tokenActual) {
        btnConfigurarSubbot.style.display = '';
        btnEliminarSubbot.style.display = '';
        divYaVinculado.querySelector('.detalle').textContent =
            'Este dispositivo está autorizado para administrar los owners de este subbot.';
    } else {
        btnConfigurarSubbot.style.display = 'none';
        btnEliminarSubbot.style.display = 'none';
        divYaVinculado.querySelector('.detalle').textContent =
            'Este navegador no es el que vinculó originalmente este subbot. No se puede administrar sus owners ni eliminarlo desde aquí.';
    }
}
...
verificarDispositivoConocido();noSoyYo.addEventListener('click', () => {
    borrarNumeroRecordado();
    idYaVinculado = null;
    tokenActual = null;
    inputNumero.value = '';
...
btn.addEventListener('click', async () => {
    ...
    try {
        ...
        idActual = d.id;
        tokenActual = d.token || null;
        guardarNumeroRecordado(numero);
        if (tokenActual) guardarTokenRecordado(numero, tokenActual);
...
btnEliminarSubbot.addEventListener('click', async () => {
    ...
    if (!tokenActual) {
        setEstado('Este navegador no está autorizado para eliminar este subbot.', 'error', false);
        return;
    }
    ...
    try {
        const r = await fetch('/subbot/' + idYaVinculado, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
...
const avisoSinToken = document.getElementById('avisoSinToken');
...
async function cargarOwners() {
    ...
    if (!tokenActual) {
        avisoSinToken.style.display = 'block';
        contenidoPanel.style.display = 'none';
        return;
    }

    avisoSinToken.style.display = 'none';
    contenidoPanel.style.display = 'block';

    try {
        const r = await fetch('/subbot/' + idActual + '/owners?token=' + encodeURIComponent(tokenActual));
        const d = await r.json();

        if (!d.ok) {
            listaOwners.innerHTML = '<li>' + (d.error || 'Error') + '</li>';
            return;
        }
        ...
        listaOwners.innerHTML = '<li>Error cargando owners.</li>';
    }
}

async function quitarOwner(numero) {
    if (!idActual || !tokenActual) return;

    try {
        await fetch('/subbot/' + idActual + '/owners', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ numero, token: tokenActual })
        });
        cargarOwners();
    } catch {}
}
...
export function registrarRutasSubbot(app) {
    ...
    app.post('/subbot/vincular', async (req, reply) => {
        ...
        try {
            const { id, codigo, token } = await crearSubbot(numero);
            return { ok: true, id, codigo, token };
        } catch (error) {
            reply.code(400);
    ...
    app.delete('/subbot/:id', async (req, reply) => {
        if (!tokenSubbotValido(req.params.id, req.body?.token)) {
            reply.code(403);
            return { ok: false, error: 'Sin permiso. Solo el dispositivo que vinculó este subbot puede eliminarlo.' };
        }
    ...
    app.get('/subbot/:id/owners', async (req, reply) => {
        if (!tokenSubbotValido(req.params.id, req.query?.token)) {
            reply.code(403);
            return { ok: false, error: 'Sin permiso. Solo el dispositivo que vinculó este subbot puede ver sus owners.' };
        }
    ...
    app.post('/subbot/:id/owners', async (req, reply) => {
        if (!tokenSubbotValido(req.params.id, req.body?.token)) {
            reply.code(403);
            return { ok: false, error: 'Sin permiso. Solo el dispositivo que vinculó este subbot puede agregar owners.' };
        }
