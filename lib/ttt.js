...
import { resolverJidReal } from './resolverJid.js';
...
export async function manejarMensajeTTT(
    ...
    // ---------------------------------------------------
    // COMPARACIÓN SIMÉTRICA (LID vs PN)
    // ---------------------------------------------------
    // No basta con resolver el JID del rival UNA vez al crear
    // la partida — el mismo contacto puede identificarse con
    // @lid en un momento y con @s.whatsapp.net en otro, incluso
    // el propio retador. Por eso aquí se resuelven los TRES
    // (quien manda el mensaje, jugador X, jugador O) en cada
    // mensaje, y se comparan ya resueltos — así no importa qué
    // formato haya quedado guardado al inicio.
    // ---------------------------------------------------

    const remitenteResuelto =
        await resolverJidReal(sock, chatJid, remitente);

    const jidXResuelto =
        await resolverJidReal(sock, chatJid, partida.jugadores.X);

    const jidOResuelto =
        await resolverJidReal(sock, chatJid, partida.jugadores.O);

    const simboloRemitente =
        jidXResuelto === remitenteResuelto
            ? 'X'
            : jidOResuelto === remitenteResuelto
                ? 'O'
                : null;
