...
export async function manejarMensajePreguntaHot(sock, msg) {
    ...
    // Solo la persona a la que le tocaba puede "responder". Se
    // resuelven AMBOS lados (quien manda y el objetivo guardado)
    // en cada mensaje — el mismo contacto puede identificarse
    // con @lid en un momento y @s.whatsapp.net en otro, así que
    // no basta con haber resuelto el objetivo solo al crear la
    // sesión.
    const remitenteResuelto = await resolverJidReal(sock, chatJid, remitente);
    const objetivoResuelto = await resolverJidReal(sock, chatJid, sesion.objetivo);

    if (remitenteResuelto !== objetivoResuelto) return false;
