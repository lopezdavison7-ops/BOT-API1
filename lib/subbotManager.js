...
//
// 🔇 SUBBOTS SILENCIADOS POR GRUPO: si un grupo está marcado
// como "subbots mudos" (database/subbotSilenciado.json), los
// subbots NO responden nada en ese grupo. El bot principal
// sigue respondiendo normal. En los demás grupos todo normal.
// ============================================================
...
import { subbotSilenciadoEnGrupo } from './subbotSilenciado.js';
...
// ============================================================
// TOKEN DEL PANEL (seguridad)
// ============================================================
// Solo el navegador que vinculó el subbot recibe este token, y
// es lo único que permite ver/agregar/quitar owners o eliminar
// el subbot desde la web. Así nadie más puede auto-proclamarse
// owner desde el panel.
// ============================================================
function archivoTokenDe(authFolder) {
    return path.join(authFolder, 'token.json');
}
...
function asegurarToken(authFolder, id) {
    const archivo = archivoTokenDe(authFolder);
    if (fs.existsSync(archivo)) return;
    try {
        const token = generarToken();
        fs.writeFileSync(archivo, JSON.stringify({ token }, null, 2));
        console.log(`[SUBBOT ${id}] 🔑 Token de panel generado (migración).`);
    } catch (error) {
        console.error('[SUBBOT] Error guardando token:', error?.message || error);
    }
}

export function tokenSubbotValido(id, token) {
    const s = subbotsActivos.get(id);
    if (!s || !token) return false;
    try {
        const datos = JSON.parse(fs.readFileSync(archivoTokenDe(s.authFolder), 'utf8'));
        return datos?.token === token;
    } catch {
        return false;
    }
}
...
async function conectarSubbot({ id, authFolder, numero, esNuevo, resolverCodigo, rechazarCodigo }) {
    try {
        // Migración: subbots antiguos sin token reciben uno aquí
        asegurarToken(authFolder, id);
        ...
        sock.ev.on('messages.upsert', async ({ messages }) => {
            ...
            // 🔇 SUBBOTS SILENCIADOS POR GRUPO: si este grupo está
            // marcado como "subbots mudos", este subbot no responde
            // nada. El bot principal no se ve afectado.
            if (subbotSilenciadoEnGrupo(m.key.remoteJid)) return;
        ...
        sock.ev.on('connection.update', async update => {
            ...
            if (connection === 'open') {
                ...
                // para subbots reconectados tras un reinicio.
                ...
                // 👑 OWNER AUTOMÁTICO
                // ====================================================
                // El propio número del subbot (el que lo vinculó)
                // siempre queda registrado en su lista de owners,
                // aunque el archivo se haya perdido, borrado o
                // nunca se haya creado bien. Sin pasos manuales.
                // ====================================================
                try {
                    const numeroPropio = String(sock.user?.id || '')
                        .split('@')[0]
                        .split(':')[0]
                        .replace(/\D/g, '');

                    if (numeroPropio) {
                        guardarOwner(numeroPropio, sock.archivoOwner);
                    }
                } catch (error) {
                    console.error(`[SUBBOT ${id}] Error asegurando owner automático:`, error?.message || error);
                }
            }
...
export function crearSubbot(numeroCrudo) {
    return new Promise((resolve, reject) => {
        ...
        // 🔑 Token del panel: solo este navegador podrá administrar
        // los owners de este subbot o eliminarlo desde la web.
        const token = generarToken();
        try {
            fs.writeFileSync(
                archivoTokenDe(authFolder),
        ...
        conectarSubbot({
            ...
            numero,
            esNuevo: true,
            resolverCodigo: codigo => resolve({ id, codigo, token }),
